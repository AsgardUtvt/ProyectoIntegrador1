...
class Ventana_Nuevo_Consultorio_Dialog(QDialog):
    _TUPLA_DATOS = ()
    def __init__(self, db: MySqlManager):
        super().__init__()
        self.db = db
        uic.loadUi("Documentacion/QtDesigner/consultorio_nuevo_dialog.ui", self)
        self.mb = Message_Box()
        self.es = GCS()
        ...
        self.pb_nuevo_consultorio.clicked.connect(lambda: self.btn_crear_consultorio())
    ...
    def btn_crear_consultorio(self):
        ...
        try:
            ...
            if len(datos_faltantes) > 0:
                ...
                try:
                    scm=SCM(c_name=nombre_consultorio,
                       c_colonia=colonia,
                    ...
                    lista_consultorio = [nombre_consultorio]
                    if self.cantidad_consultorio_duplicado(lista_consultorio) > 0:
                        self.mb.message_box(self, "error", "Ya existe esa clinica ", "Ya hay una clínica con ese nombre")
                    else:
                        if scm.insertar_datos(self.db):
    ...
    def cantidad_consultorio_duplicado(self, lista_dato):
        cantidad_consultorio = GCS.encontrar_duplicados_sub_consultorio(self.db, lista_dato)
