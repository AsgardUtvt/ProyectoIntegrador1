...
from PyQt6.QtWidgets import QApplication, QMainWindow, QPushButton, QMessageBox, QWidget, QDialog
...
class Ventana_Modificar_Sub_Consultorio_Dialog(QDialog):
    _TUPLA_DATOS = ()
    def __init__(self, db: MySqlManager, val_int):
        super().__init__()
        self.db = db
        self.val_int = val_int
        uic.loadUi("Documentacion/QtDesigner/consultorio_modificar_dialog.ui", self)
        self.mb = Message_Box()
        self.es = GCS()
    ...
    def btn_crear_consultorio(self):
        ...
        try:
            ...
            if len(datos_faltantes) > 0:
                ...
                try:
                    if SCM.actualizar_datos(self.db, name_update=nombre_consultorio, calle_update=calle, colonia_update=colonia, num_ext_update=num_exeterior, num_int_update=num_interior, localidad_update=localidad, id_estado_update=digitos, tel_update=telefono, tel_dos_update=telefono_dos, municipio_update=municipio, cp_update=cp):
    ...
    def showEvent(self, event):
        ...
        lista_consultorio= [AIUCSG.obetner_id_consultorio(), self.val_int]
        obtener_datos_rellenar = GCS.obtener_datos_sub_conusltorio_modificar(self.db, lista_consultorio)
        if obtener_datos_rellenar:
            label, combo, num = "No encontrado", "1 No Asignado", "000"
            print("datos obtenidos: ", obtener_datos_rellenar)
            self.le_nombre_consultorio.setText(obtener_datos_rellenar.get("S_NAME", label))
            self.le_calle.setText(obtener_datos_rellenar.get("S_CALLE",label ))
            self.le_num_exterior.setText(obtener_datos_rellenar.get("S_EXT", num))
            self.le_num_interior.setText(obtener_datos_rellenar.get("S_INT", num))
            self.le_localidad.setText(obtener_datos_rellenar.get("S_LOCALIDAD", label))
            self.le_cp.setText(obtener_datos_rellenar.get("S_CP", num))
            self.le_municipio.setText(obtener_datos_rellenar.get("S_MUN", label))
            self.le_colonia.setText(obtener_datos_rellenar.get("S_COLONIA", label))
            self.le_telefono.setText(obtener_datos_rellenar.get("S_TEL", num))
            self.le_telefono_dos.setText(obtener_datos_rellenar.get("S_TEL_DOS", num))
            estado_txt = obtener_datos_rellenar.get("S_EST", combo)
